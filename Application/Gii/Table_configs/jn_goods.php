...
return array(
	...
	'pk' => 'id',    // 表中主键字段名称
	/********************* 要生成的模型文件中的代码 ******************************/
	'insertFields' => "array('goods_name','cate_id','brand_id','type_id','market_price','shop_price','jifen','jyz','jifen_price','promote_price','promote_start_time','promote_end_time','is_hot','is_new','is_best','seo_keyword','seo_description','sort_num','goods_desc','is_on_sale','is_delete')",
	'updateFields' => "array('id','goods_name','cate_id','brand_id','type_id','market_price','shop_price','jifen','jyz','jifen_price','promote_price','promote_start_time','promote_end_time','is_hot','is_new','is_best','seo_keyword','seo_description','sort_num','goods_desc','is_on_sale','is_delete')",
	'validate' => "
		...
		array('promote_start_time', 'require', '促销开始时间不能为空！', 1, 'regex', 3),
		array('promote_start_time', 'number', '促销开始时间必须是一个整数！', 1, 'regex', 3),
		array('promote_end_time', 'require', '促销结束时间不能为空！', 1, 'regex', 3),
	...
	'fields' => array(
		...
		'jifen_price' => array(
			'text' => '如果要用积分兑换,需要的积分数,0代表不能用积分兑换',
			'type' => 'text',
			'default' => '0',
		),
		'promote_price' => array(
	...
	'search' => array(
		...
		array('cate_id', 'normal', '', 'eq', '主分类id'),
		array('brand_id', 'normal', '', 'eq', '品牌的id'),
		array('type_id', 'normal', '', 'eq', '商品类型的ID'),
		array('shop_price', 'between', 'shop_pricefrom,shop_priceto', '', '本店商品价格'),
